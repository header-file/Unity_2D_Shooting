...
public class Bullet : MonoBehaviour
{
    ...
    public void BloodSuck(float dmg)
    {
        ...
        {
            float heal = dmg * (GameManager.Inst().Player.GetItem(GameManager.Inst().UpgManager.BData[GameManager.Inst().Player.GetBulletType()].GetEquipIndex()).Value / 1000.0f);
            if (heal < 1)
                heal = 1;
        ...
        else
        {
            ...
            float heal = dmg * (GameManager.Inst().Player.GetItem(GameManager.Inst().UpgManager.BData[sub.GetBulletType()].GetEquipIndex()).Value / 1000.0f);
            if (heal < 1)
                heal = 1;
